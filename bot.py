...
def ensure_default_account_if_empty():
    label = os.environ.get("DEFAULT_LOGIN")
    token = os.environ.get("DEFAULT_PASSWORD")
    max_cc = int(os.environ.get("DEFAULT_MAX_SEATS", "5"))

    if not label or not token:
        return None  # nothing to create

    with db() as con:
        # Create only if the table is empty (race-safe in SQLite)
        con.execute(
            """
        ...
        latest = con.execute(
            "SELECT id, max_concurrent, allocated_count FROM accounts ORDER BY id DESC LIMIT 1"
        ).fetchone()
    return latest
...
def get_seat_availability():
    ...
    if not latest:
        # Try to create a default account once
        latest = ensure_default_account_if_empty()
        if not latest:
            # No env vars provided → still return no seats
            return 0, None
...
contact_conv = ConversationHandler(
    ...
    states={
        CONTACT: [
            MessageHandler(
                (filters.TEXT | filters.PHOTO | filters.Document.MimeType("application/pdf"))
                & ~filters.COMMAND,
                contact_receive,
            )
        ],
...
def health():
    ...
    return "Timed Access Bot is alive ✅", 200
...
def _start_ptb():
    ...
    _ptb_loop.call_soon_threadsafe(lambda: None)
    ...
    if PUBLIC_BASE_URL:
        ...
        log.warning("PUBLIC_URL not set, webhook not configured.")
...
# Start PTB immediately (instead of before_first_request)
_start_ptb()
