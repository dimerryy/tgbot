...
import os, re, sqlite3, logging, random, string
...
PAYMENT_PHONE = os.environ.get("PAYMENT_PHONE", "+77776952267").strip()


SCHEMA_SQL = """
CREATE TABLE IF NOT EXISTS accounts (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  label TEXT NOT NULL,        -- email/username
  token TEXT NOT NULL,        -- temporary password
  is_allocated INTEGER NOT NULL DEFAULT 0, -- legacy flag (kept for backward compat)
...
def quote_price(minutes: int) -> Quote:
    """
    Tiered pricing based on billed hours (30-min blocks):
      1h+:  0% off
      2h+:  5% off
      4h+: 10% off
      8h+: 20% off
    ...
    # bill in 30-min blocks
    blocks = (minutes + 29) // 30
    billed_hours = blocks * 0.5

    # choose the highest matching tier
    discount = 0.0
...
def allocate_account() -> sqlite3.Row | None:
    # pick ONLY the most recently added account (highest id)
    with db() as con:
        ...
        # return fresh row after increment
...
async def copy_ref_cb(update, context):
    query = update.callback_query
    # pull the current reference from user_data if present
...
# ---- Single-account helpers from previous step (optional to keep) ----
def expire_all_active_sessions_and_notify(context: ContextTypes.DEFAULT_TYPE, note: str = "Access has ended."):
    with db() as con:
...
async def confirm_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Ask for bill/receipt upload before allocating seat
...
async def monitor_gmail(update, context):
    ...
    # Get current latest message id (Inbox only is often nicer: add labelIds=['INBOX'] if you want)
    ...
    while True:
        ...
        if 'messages' not in res:
            continue  # inbox empty

        new_id = res['messages'][0]['id']
        if latest_id and new_id == latest_id:
            continue  # no new mail yet

        # Fetch just the headers we need; 'Subject' will be in payload.headers
...
async def receive_bill(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Forward/copy the bill to admin with a caption
    if ADMIN_ID:
        caption = (
...
async def admin_list_seats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # query active sessions + account label
    with db() as con:
        rows = con.execute(
            """
    ...
    lines = []
    for r in rows:
        # try to pretty-print the user as @username if available
        try:
...
def get_seat_availability():
    """
    Returns:
      free_seats: int (free seats on the NEWEST account only)
      eta_minutes: int | None (earliest minutes until a seat frees on the NEWEST account)
    """
    now_ts = int(datetime.now(tz=TIMEZONE).timestamp())
    with db() as con:
...
async def admin_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Append a NEW account (do NOT expire sessions; do NOT delete old rows)
    with db() as con:
        con.execute(
    ...
    # Tell admin which account is now the allocator's target
    with db() as con:
...
def cancel_expiry_job(session_id: int, application):
    """Cancel the scheduled expiry job for a session (name pattern: expire:<id>)."""
    name = f"expire:{session_id}"
...
async def notify_users(context, user_ids, text: str):
    """DM a list of users; ignore failures."""
...
def users_on_accounts(account_ids):
    """Return (session_id, telegram_id) for active sessions on given account ids."""
    if not account_ids:
        return []
...
def all_active_sessions():
    """Return (session_id, telegram_id) for all active sessions."""
    with db() as con:
        rows = con.execute(
...
async def admin_purge(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # No args -> purge ALL accounts & sessions
    if not context.args:
        active = all_active_sessions()                # [(sid, uid), ...]
        # expire & delete
        with db() as con:
            con.execute("UPDATE sessions SET status='expired' WHERE status='active'")
    ...
    # Case A: /admin_purge #<session_id>  or /admin_purge <session_id>
    if arg.startswith("#") or arg.isdigit():
        ...
        try:
            await context.bot.send_message(
                ...
                text=f"Your session was ended by the admin. Please log out."
            )
        except Exception:
            pass
    ...
    # Case B: /admin_purge <login>  (purge ONE login/email)
    target_label = arg
    with db() as con:
    ...
    # collect affected sessions/users first
    affected = users_on_accounts(account_ids)         # [(sid, uid), ...]
    # expire sessions and delete those account rows
    q = ",".join("?" for _ in account_ids)
    with db() as con:
        ...
        # recompute allocated_count for remaining accounts
        con.execute("UPDATE accounts SET allocated_count = 0")
        con.execute("""
        WITH active AS (
          SELECT account_id, COUNT(*) AS c
...
async def contact_start(update, context):
    # Users type /contact to start
    if not ADMIN_ID:
        await update.message.reply_text("Admin is not configured. Please try again later.")
        return ConversationHandler.END

    await update.message.reply_text(
        "Send your message for the admin (text, photo, or PDF). "
        "When you’re done, you can /cancel."
    )
    return CONTACT

async def contact_receive(update, context):
    # Accept text, photos, PDFs; forward to admin with metadata
    if not ADMIN_ID:
        await update.message.reply_text("Admin is not configured. Please try again later.")
        return ConversationHandler.END
    ...
    try:
        if update.message.photo or (
            update.message.document and update.message.document.mime_type == "application/pdf"
        ):
            # Copy the original message to admin (keeps the media)
            await context.bot.copy_message(
                chat_id=int(ADMIN_ID),
                from_chat_id=update.message.chat_id,
                message_id=update.message.message_id,
            ...
            # Send a meta line so admin knows who sent it
            await context.bot.send_message(
                chat_id=int(ADMIN_ID),
                text=f"From {tag} (user id {user.id}) via /contact"
            )
        else:
            # Text or other content: send as text
        ...
        logging.getLogger("timed_access_bot").warning("contact forward failed: %s", e)
...
def main():
    ...
    states={
        CONTACT: [MessageHandler((filters.TEXT | filters.PHOTO | filters.Document.PDF) & ~filters.COMMAND, contact_receive)],
    },
    fallbacks=[CommandHandler("cancel", cancel)],
)
    ...
    app.add_handler(CommandHandler("my_session", my_session))
    app.add_handler(CommandHandler("help", help_cmd))
    app.add_handler(CommandHandler("admin_add", admin_add))
    app.add_handler(CommandHandler("admin_set_seats", admin_set_seats))
    app.add_handler(CommandHandler("admin_list", admin_list))
    app.add_handler(CommandHandler("admin_purge", admin_purge))
    app.add_handler(CallbackQueryHandler(copy_phone_cb, pattern="^copy_phone$"))
    app.add_handler(CallbackQueryHandler(copy_ref_cb, pattern="^copy_ref$"))
    app.add_handler(CommandHandler("admin_list_seats", admin_list_seats))
